...
| — | **Peripherals subtotal** | — | **3,979,911** | **~$153** | Still open: masking tape (~10k), matte mat (~100–250k), side cutters (~30–60k), kitchen scale (~60–120k), 2× USB-A→C adapters (~60–160k), muffin cups + white tray + bowl/reject cups (~70k), mì ly cups + PET bottles + caps (BHX, ~100k), M3×10/12 spare screws (~20k), spare gripper servos + spare Waveshare driver board (AliExpress, ~$40–50 — longest lead, order first), high-CRI lamp (deferred to coffee phase). |
