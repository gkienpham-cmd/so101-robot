...
5. **The handed-off project venv was pipless and missing the camera extra** — the existing `uv`-created Python 3.12.4 environment still ran pytest and Ruff, but `python -m pip` and `import cv2` failed. Fix: bootstrap the bundled pip with `python -m ensurepip --upgrade`, then install `-e '.[dev,camera]'`. Verification: OpenCV 4.13.0 imports, all 31 tests pass, and Ruff is green. Lesson: executable entry points can survive in a venv even when an optional runtime dependency is absent, so verify imports rather than inferring readiness from scripts in `.venv/bin`.
6. **The first formal camera soak passed numerically but saved a black C270 startup frame** — the C270 averaged healthy brightness over the run, yet the preflight preserved AVFoundation's first all-zero frame for visual review. Fix: retain the final successful frame instead; a 10-second dual-camera smoke test then produced two usable samples. The next 30-minute run failed after the camera connection/scene changed, so the formal camera gate remains open and both failed attempts are preserved rather than combined into a synthetic pass.
