# SO-101 Real-World Applications in Vietnam and HCMC
...
| 5 | **Bottle-Cap Color Sorter** (day-1 shakedown) | **6.0/10** | Low | **TIER 1 — mandatory first real-arm task** |
...
Recommended sequence (hardware lands Jul 20–25):
**Week 1: Cap Sorter shakedown → Weeks 2–3: Battery Pull-Out + 3-Stream Sorter → Weeks 3–5: Mì Gói v1 (dry triad) → Weeks 5–8: Mock Feeding → Month 2–3: Workshop Kit** (which reuses the cap sorter as its curriculum).

---

### 3.1 Bottle-Cap Color Sorter — the mandatory day-1 shakedown (6.0/10)
...
- Top cam + wrist cam at 640×480; record with `--dataset.streaming_encoding=true --dataset.encoder_threads=2` (the macOS crash workaround — `parallel_encoding` is a Python-API parameter, not a v0.6.0 CLI flag; see doc 04 §6.5)
